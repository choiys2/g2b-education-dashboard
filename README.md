...
방을 여는 순간(1탭) 그 비서가 담당하는 내용이 자동으로 뜬다. 상단 **실행**을 따로
누르지 않아도 된다 — 커넥터를 쓸 수 있는 화면(Artifact로 발행된 경우)이면 실제
조회 결과가, 커넥터가 없는 화면(로컬에서 그냥 `index.html`을 열거나 GitHub Pages
정적 배포본)이면 저장된 대본이 자동 재생된다. 같은 방은 세션당 한 번만 자동
실행하고, 다시 보려면 실행 버튼의 "다시 조회"를 쓴다. 칩을 누르면 그 항목 하나만
즉시 다시 조회한다.
