...
Artifact로 발행해 커넥터를 쓸 수 있는 화면에서는, 방을 여는 순간(1탭) 그 비서가
담당하는 조회를 전부 자동으로 돌려 상단 **실행**을 따로 누르지 않아도 결과가 바로
뜬다. 같은 방은 세션당 한 번만 자동 조회하고(재조회는 실행 버튼의 "다시 조회"),
칩을 누르면 그 항목 하나만 즉시 다시 조회한다. 커넥터를 쓸 수 없는 화면
(GitHub Pages 정적 배포본 등)에서는 자동 재생을 걸지 않고 상단 실행을 눌러야
대본이 재생된다 — 지어낸 값을 실제 조회 결과처럼 자동으로 보여주지 않기 위해서다.
